...
famBoxPlot <- var1 %>% 
  ...
  full_join(var6) %>%
  full_join(var7)
famBoxPlot$group <- factor(famBoxPlot$group, levels = c("2016Q96K97", "2016K97Gfs", "2016PSEN2", "2017Q96K97", "2018PSEN2", "2019Q96K97", "2019SORL1"))
famBoxPlot %>%
...
animation <- anim %>%
  ...
  geom_point(size = 6, stroke = 1, alpha = 0.8) +
  labs(colour = "Family", shape = "Provider") +
  xlab(label = "PC1") +
  ylab(label = "PC2") +
  coord_cartesian(
  ...
  theme(plot.title = element_text(size = 40, face = "bold"),
        # legend.key.height = unit(4, units = "cm"),
        # legend.key.width = unit(2, units = "cm")
        legend.title = element_text(size = 30),
        legend.text = element_text(size = 20),
        axis.title = element_text(size = 20))
# animate(animation, height = 800, width = 1000, rewind = TRUE, nframes = 200)
# anim_save("../files/animation.gif")
```
